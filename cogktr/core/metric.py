...
class Link_Prediction(object):
    ...
    def _caculate_rank(self,single_sample,entity_type):
        ...
        expanded_triplet=single_triplet.expand(self.node_dict_len, 3,self._single_batch_len)#(self._node_dict_len,3,self._single_batch_len)
        ...
        if entity_type=="head":
            ...
            expanded_node=expanded_node.expand(self.node_dict_len, 1,self._single_batch_len)#(self._node_dict_len,1,self._single_batch_len)
            expanded_triplet = torch.cat([expanded_node, original_r_t], dim=1)#(self._node_dict_len,3,self._single_batch_len)
        if entity_type=="tail":
        ...
        if self.link_prediction_filt:
            ...
            for i,triplet_key in enumerate(triplet_key_list):
                ...
                if triplet_key in self._correct_train_node_dict[entity_type].keys():
                    valid_correct_node_index_list=torch.where(sorted_score_rank[:,i] ==torch.unsqueeze(torch.tensor(self._correct_train_node_dict[entity_type][triplet_key]),dim=1))[1]+1
                if triplet_key in self._correct_valid_node_dict[entity_type].keys():
                    train_correct_node_index_list=torch.where(sorted_score_rank[:,i] == torch.unsqueeze(torch.tensor(self._correct_valid_node_dict[entity_type][triplet_key]),dim=1))[1]+1
                if self.test_dataset!=None:
    ...
    def caculate(self,model,current_epoch):
        ...
        if self.link_prediction_raw:
            ...
            self._current_result["Raw_Hits1"]= round((torch.sum((current_raw_rank <= 1)) / (2 * len(metric_dataset)) * 100).item(), 3)
            self._current_result["Raw_Hits3"]= round((torch.sum(current_raw_rank <= 3) / (2 * len(metric_dataset)) * 100).item(), 3)
        ...
        if self.link_prediction_filt:
            ...
            self._current_result["Filt_Hits3"]= round((torch.sum(current_filt_rank <= 3) / (2 * len(metric_dataset)) * 100).item(), 3)
            self._current_result["Filt_Hits10"]= round((torch.sum(current_filt_rank<= 10) / (2 * len(metric_dataset)) * 100).item(), 3)
    ...
    def log(self):
        if self.link_prediction_raw:
            self.logger.info("{} Epoch {}/{}  Raw_Hits@1:{}   Raw_Hits@3:{}   Raw_Hits@10:{}   Raw_MR:{}   Raw_MRR:{}".format(
                self.metric_type,self._current_epoch, self.total_epoch, self._current_result["Raw_Hits1"], self._current_result["Raw_Hits3"], self._current_result["Raw_Hits10"], self._current_result["Raw_MR"], self._current_result["Raw_MRR"]))
        if self.link_prediction_filt:
            self.logger.info("{} Epoch {}/{}  Filt_Hits@1:{}   Filt_Hits@3:{}   Filt_Hits@10:{}   Filt_MR:{}   Filt_MRR:{}   ".format(
                self.metric_type,self._current_epoch, self.total_epoch, self._current_result["Filt_Hits1"], self._current_result["Filt_Hits3"], self._current_result["Filt_Hits10"], self._current_result["Filt_MR"], self._current_result["Filt_MRR"]))
    def write(self):
        if self.writer!=None:
            if self.link_prediction_raw:
                self.writer.add_scalars("Hits@1", {"{}_Raw_Hits@1".format(self.metric_type): self._current_result["Raw_Hits1"]}, self._current_epoch)
                self.writer.add_scalars("Hits@3", {"{}_Raw_Hits@3".format(self.metric_type): self._current_result["Raw_Hits3"]}, self._current_epoch)
                self.writer.add_scalars("Hits@10", {"{}_Raw_Hits@10".format(self.metric_type): self._current_result["Raw_Hits10"]}, self._current_epoch)
                self.writer.add_scalars("MR", {"{}_Raw_MR".format(self.metric_type): self._current_result["Raw_MR"]}, self._current_epoch)
                self.writer.add_scalars("MRR", {"{}_Raw_MRR".format(self.metric_type): self._current_result["Raw_MRR"]}, self._current_epoch)
            if self.link_prediction_filt:
                self.writer.add_scalars("Hits@1", {"{}_Filt_Hits@1".format(self.metric_type): self._current_result["Filt_Hits1"]}, self._current_epoch)
                self.writer.add_scalars("Hits@3", {"{}_Filt_Hits@3".format(self.metric_type): self._current_result["Filt_Hits3"]},self._current_epoch)
                self.writer.add_scalars("Hits@10", {"{}_Filt_Hits@10".format(self.metric_type): self._current_result["Filt_Hits10"]},self._current_epoch)
                self.writer.add_scalars("MR", {"{}_Filt_MR".format(self.metric_type): self._current_result["Filt_MR"]}, self._current_epoch)
                self.writer.add_scalars("MRR", {"{}_Filt_MRR".format(self.metric_type): self._current_result["Filt_MRR"]}, self._current_epoch)
