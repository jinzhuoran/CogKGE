...
def time(func):
    def inner(*args, **kwargs):
        ...
        model=args[0]
        start=kwargs["batch"]["start"].to(model.model_device)
        end=kwargs["batch"]["end"].to(model.model_device)
        if not model.init_time_adapter:
            model.time_transfer=nn.Embedding(num_embeddings=model.time_dict_len, embedding_dim=10).to(model.model_device)
            model.init_time_adapter=True
        strat_embedding=model.time_transfer(start)
        end_embedding=model.time_transfer(end)
