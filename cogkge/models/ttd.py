...
class TransE_Add_Time(nn.Module):
    def __init__(self, entity_dict_len, relation_dict_len, embedding_dim, time_lut, p=1.0):
        ...
        self.time_transfer_matrix = nn.Embedding(num_embeddings=len(self.time_lut),
    ...
    def forward(self, triplet_idx):
        ...
        head_embedding = torch.bmm(head_embedding,
                                   self.time_transfer_matrix(start).view(-1, self.embedding_dim, self.embedding_dim) +
                                   self.time_transfer_matrix(end).view(-1, self.embedding_dim, self.embedding_dim))
        tail_embedding = torch.bmm(tail_embedding,
                                   self.time_transfer_matrix(start).view(-1, self.embedding_dim, self.embedding_dim) +
                                   self.time_transfer_matrix(end).view(-1, self.embedding_dim, self.embedding_dim))
...
class TransE_Add_Type(nn.Module):
    def __init__(self, entity_dict_len, relation_dict_len, embedding_dim, node_lut, p=1.0):
        ...
        self.type_transfer_matrix = nn.Embedding(num_embeddings=self.type_len,
                                                 embedding_dim=self.embedding_dim * self.embedding_dim)
    ...
    def forward(self, triplet_idx):
        ...
        head_embedding = torch.bmm(head_embedding, self.type_transfer_matrix(head_type).view(-1, self.embedding_dim,
                                                                                             self.embedding_dim))
        tail_embedding = torch.bmm(tail_embedding, self.type_transfer_matrix(tail_type).view(-1, self.embedding_dim,
                                                                                             self.embedding_dim))
